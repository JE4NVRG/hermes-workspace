...
- Repetição com a mesma `idempotency_key` não duplica recursos nem efeitos.
...
- Nenhum projeto fica `ready` sem verificação completa.
...
| -------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| Jean (sponsor/admin)       | Decidir risco, custo e efeitos irreversíveis | aprovar ou rejeitar operações e exceções; autorizar go-live                               | receber senha em UI/log; contornar evidências de Gate 6                   |
| Luna (control-plane owner) | Orquestrar ciclo e gates                     | iniciar dry-run, revisar evidências, aprovar quando autorizada, interromper e reconciliar | executar plano alterado após aprovação; expor secret                      |
| Gerente                    | Governança, status e risco                   | acompanhar KPIs, gates, dependências e migrações                                          | executar DDL, gerenciar secret ou autoatestar segurança                   |
...
| Security                   | Least privilege e análise de ameaça          | revisar plano, grants, exposição e provas negativas                                       | aprovar sem evidência; operar com segredo em prompt                       |
| QA                         | Prova funcional e de isolamento              | executar testes em ambiente autorizado e registrar evidência                              | marcar GO com teste omitido ou mock                                       |
| Operador do Project Center | Execução controlada                          | executar plano aprovado sob identidade de serviço restrita                                | alterar escopo durante a execução                                         |
| Agente externo             | Consumir contexto do projeto                 | ler manifesto, schema exportado, contratos e URL mascarada conforme scope                 | obter admin DSN, JWT secret, env real ou acesso ao banco de outro projeto |
...
- secret em `/home/jean/.config/je4ndev/projects/<slug>.env`, modo `0600`;
...
executing/verifying → rollback_pending → rolling_back → rolled_back
executing/verifying → reconciliation_required → verifying → ready|rolled_back
qualquer estado não terminal → cancelled (somente se não houver efeito pendente)
```

Cada transição registra actor, timestamp, motivo, `operation_id`, `idempotency_key`, hash do plano e referências de evidência. Eventos não carregam secrets.

### 7.2 Etapa 1 — Dry-run

Entrada mínima:

- `client_id`, `project_id`, `slug` e ambiente;
- modo solicitado e capacidades necessárias;
- classificação de dados;
- estimativa de recursos;
- política de backup/restore;
- `idempotency_key` fornecida pelo cliente;
...
- Repetir a mesma chave retorna a operação original e seu estado.
- Mesma chave com payload/hash diferente é conflito, nunca nova execução.
...
O projeto só fica `ready` após, no mínimo:
...
Falha de verificação impede `ready`, ainda que a execução técnica tenha terminado.

### 7.6 Etapa 5 — Rollback e reconciliação

- Rollback usa compensações definidas no plano aprovado.
- Artefato com dado pode ser colocado em quarentena antes de remoção; não se presume que `DROP` seja seguro.
- Se reversão automática for insegura ou impossível, o estado vira `reconciliation_required`, bloqueia novas mutações e apresenta runbook e owner.
- Depois do rollback, as provas negativas e o inventário são executados novamente.
- `rolled_back` significa estado verificado, não apenas comando de compensação enviado.
...
| RF-05 | Operações mutáveis exigem `idempotency_key` e não duplicam recursos em retry.                                  | P0         |
| RF-06 | Aprovação registra identidade, hash, prazo, escopo e decisão.                                                  | P0         |
| RF-07 | Executor aplica lock por projeto e impede concorrência incompatível.                                           | P0         |
| RF-08 | Estado parcial dispara rollback automático seguro ou reconciliação bloqueante.                                 | P0         |
...
| RF-19 | Ações destrutivas exigem confirmação contextual e aprovação segregada.                                         | P0         |
...
| Projetos marcados `ready` com prova cruzada A/B aprovada       |                                                        100% | por criação/migração                |
| Respostas/logs com secret real                                 |                                                           0 | contínuo; scanners/testes negativos |
| Projetos com backup dentro da política                         |                                                       ≥ 99% | diário                              |
| Projetos com restore test válido                               | 100% antes do `ready`; ≥ 95% dentro da periodicidade depois | diário/mensal                       |
| Rollbacks automáticos seguros concluídos dentro do runbook     |                                                       ≥ 95% | trimestral                          |
| Operações em `reconciliation_required` sem owner/SLA           |                                                           0 | contínuo                            |
| Lead time mediano dry-run→ready em `postgresql_isolated`       |                           ≤ 30 min, excluindo espera humana | mensal                              |
...
| R-04 | Retry cria database, role, volume ou stack duplicada                      | alto/médio            | idempotency key, hash, locks e reconciliação                                             | teste de retry falha                           |
| R-05 | Aprovação aplicada a plano diferente                                      | crítico/baixo         | hash imutável, expiração e re-dry-run                                                    | hash/estado divergente                         |
| R-06 | Falha parcial deixa recurso órfão ou inseguro                             | alto/médio            | compensações, quarantine e `reconciliation_required`                                     | ausência de rollback/runbook/owner             |
...
| R-14 | Rollback destrutivo apaga dado válido                                     | crítico/baixo         | classificação reversível/compensável, quarantine e aprovação                             | compensação sem proteção de dado               |
...
- Fluxo dry-run→aprovação→execução→verificação→rollback definido.
...
- Semântica de idempotência, locks, hash, expiração e concorrência definida.
...
- Rollback/reconciliação exercitados em falhas injetadas.
...
- Mesma `idempotency_key` repetida não duplica nenhum recurso.
...
7. Projeto só muda para `ready` após todas as verificações obrigatórias.
8. Testes A→B e B→A falham com acesso negado.
9. Agente obtém contexto útil e URL mascarada, mas nenhuma credencial real.
10. Falha intermediária resulta em `rolled_back` verificado ou `reconciliation_required` com lock, owner e runbook.
...
- Threat model e matriz RBAC detalhados.
